...
# =============================================================================
# MATRIX DE 6 CAMARAS CON WATCHDOG v2.6 - VPN Traffic Monitor + Smart Restart
# =============================================================================
# Mejoras v2.6:
#   - Monitorea trafico de red por proceso MPV (detecta streams sin datos)
#   - Reinicio preventivo cada 2h SOLO para camaras VPN (CAM5, CAM6)
#   - Mantiene reinicio general cada 4h para camaras locales
#   - Detecta peer Tailscale idle que causa congelamiento
# =============================================================================
...
VPN_RESTART_INTERVAL=7200    # 2 horas - reinicio preventivo CAM5/CAM6
CPU_THRESHOLD=2.0
FROZEN_CYCLES=3
ROUTER_IP="192.0.2.1"
TRAFFIC_CHECK_INTERVAL=60    # Segundos entre checks de trafico
MIN_BYTES_PER_INTERVAL=50000 # Minimo ~50KB en 60s para considerar activo
NO_TRAFFIC_CYCLES=2          # Ciclos sin trafico para declarar congelado

# --- CONFIGURACION DE CAMARAS ---
declare -A CAM_URLS CAM_TITLES CAM_X CAM_Y CAM_W CAM_H CAM_LOW_CPU_COUNT CAM_IS_VPN
declare -A CAM_LAST_BYTES CAM_NO_TRAFFIC_COUNT
...
CAM_TITLES[1]="CAM1-NVR";     CAM_X[1]=0;    CAM_Y[1]=0;   CAM_W[1]=640; CAM_H[1]=540; CAM_IS_VPN[1]=0
CAM_URLS[1]="${CAM_URLS[1]:-rtsp://USER:PASS@192.0.2.40:554/cam/realmonitor?channel=1&subtype=1}"

CAM_TITLES[2]="CAM2-NVR";     CAM_X[2]=640;  CAM_Y[2]=0;   CAM_W[2]=640; CAM_H[2]=540; CAM_IS_VPN[2]=0
CAM_URLS[2]="${CAM_URLS[2]:-rtsp://USER:PASS@192.0.2.40:554/cam/realmonitor?channel=2&subtype=1}"

CAM_TITLES[3]="CAM3-NVR";     CAM_X[3]=1280; CAM_Y[3]=0;   CAM_W[3]=640; CAM_H[3]=540; CAM_IS_VPN[3]=0
CAM_URLS[3]="${CAM_URLS[3]:-rtsp://USER:PASS@192.0.2.40:554/cam/realmonitor?channel=3&subtype=1}"

CAM_TITLES[4]="CAM4-ICSEE";   CAM_X[4]=0;    CAM_Y[4]=540; CAM_W[4]=640; CAM_H[4]=540; CAM_IS_VPN[4]=0
CAM_URLS[4]="${CAM_URLS[4]:-rtsp://USER:PASS@192.0.2.40:554/cam/realmonitor?channel=4&subtype=1}"

CAM_TITLES[5]="CAM5-LUGAR2"; CAM_X[5]=1280; CAM_Y[5]=540; CAM_W[5]=640; CAM_H[5]=540; CAM_IS_VPN[5]=1
CAM_URLS[5]="${CAM_URLS[5]:-rtsp://USER:PASS@198.51.100.30/ch0_1.h264}"

CAM_TITLES[6]="CAM6-LUGAR2"; CAM_X[6]=640;  CAM_Y[6]=540; CAM_W[6]=640; CAM_H[6]=540; CAM_IS_VPN[6]=1
...
# ============= NUEVO: Obtener bytes recibidos por proceso =============
get_process_rx_bytes() {
    local title="$1"
    local pid=$(pgrep -f "title=$title" | head -1)
    [ -z "$pid" ] && echo "0" && return

    # Sumar bytes recibidos de todos los file descriptors de red del proceso
    local total_bytes=0
    if [ -d "/proc/$pid/fd" ]; then
        for fd in /proc/$pid/fd/*; do
            if [ -L "$fd" ]; then
                ...
                if [[ "$link" == socket:* ]]; then
                    # Obtener inode del socket
                    local inode=$(echo "$link" | grep -oP '\d+')
                    # Buscar en /proc/net/tcp y tcp6 los bytes recibidos
                    local rx=$(grep -h "$inode" /proc/net/tcp /proc/net/tcp6 2>/dev/null | awk '{print $8}' | head -1)
                    if [ -n "$rx" ]; then
                        # Convertir de hex a decimal
                        local rx_dec=$((16#$rx))
                        total_bytes=$((total_bytes + rx_dec))
                    fi
                fi
            fi
        done
    ...
    # Metodo alternativo: usar /proc/pid/net/dev si existe
    if [ $total_bytes -eq 0 ] && [ -f "/proc/$pid/net/dev" ]; then
        total_bytes=$(awk 'NR>2 {sum+=$2} END {print sum+0}' /proc/$pid/net/dev 2>/dev/null)
    fi

    # Metodo mas simple: verificar si el proceso esta recibiendo datos via strace (solo debug)
    # Por ahora usamos el contador de bytes del sistema para el PID
    if [ $total_bytes -eq 0 ]; then
        # Usar /proc/pid/io si existe
        if [ -f "/proc/$pid/io" ]; then
            total_bytes=$(grep "^read_bytes:" /proc/$pid/io 2>/dev/null | awk '{print $2}')
...
# ============= NUEVO: Verificar si camara tiene trafico de red =============
check_camera_traffic() {
    local cam_num=$1
    local title="${CAM_TITLES[$cam_num]}"

    local current_bytes=$(get_process_rx_bytes "$title")
    local last_bytes=${CAM_LAST_BYTES[$cam_num]}

    # Calcular diferencia
    local diff=$((current_bytes - last_bytes))

    # Actualizar ultimo valor
    CAM_LAST_BYTES[$cam_num]=$current_bytes

    # Si es la primera medicion, no podemos comparar
    if [ $last_bytes -eq 0 ]; then
        return 1  # No congelada (primera medicion)
    fi

    # Verificar si hay trafico suficiente
    if [ $diff -lt $MIN_BYTES_PER_INTERVAL ]; then
        ((CAM_NO_TRAFFIC_COUNT[$cam_num]++))
        if [ ${CAM_NO_TRAFFIC_COUNT[$cam_num]} -ge $NO_TRAFFIC_CYCLES ]; then
            log "DEBUG: ${title} sin trafico por ${NO_TRAFFIC_CYCLES} ciclos (diff=$diff bytes)"
            return 0  # Congelada por falta de trafico
        fi
    else
        CAM_NO_TRAFFIC_COUNT[$cam_num]=0
    fi
...
start_all_cameras() {
    ...
    log "Camaras iniciadas"
    sleep 3
    check_and_fix_positions
}

# ============= NUEVO: Reiniciar solo camaras VPN =============
restart_vpn_cameras() {
    log "=== REINICIO PREVENTIVO CAMARAS VPN (cada 2h) ==="
    for cam_num in 5 6; do
        if [ "${CAM_IS_VPN[$cam_num]}" = "1" ]; then
            log "Reiniciando ${CAM_TITLES[$cam_num]} (VPN)..."
            start_camera "$cam_num"
            sleep 2
            fix_window_position "$cam_num"
        fi
...
watchdog_loop() {
    log "=== WATCHDOG v2.6 ACTIVO ==="
    log "    - Trafico check: <${MIN_BYTES_PER_INTERVAL} bytes en ${TRAFFIC_CHECK_INTERVAL}s = congelado"
    log "    - CPU check: <${CPU_THRESHOLD}% por ${FROZEN_CYCLES} ciclos = congelado"
    log "    - Network monitor: ping -c 3 $ROUTER_IP"
    log "    - Reinicio VPN (CAM5,CAM6): cada $((VPN_RESTART_INTERVAL/3600))h"
    ...
    while true; do
        ...
        ((TRAFFIC_CHECK_COUNTER++))

        # ============= CHECK 1: Conectividad de red =============
        if ! check_network; then
            if [ $NETWORK_WAS_DOWN -eq 0 ]; then
                log "RED CAIDA - Esperando reconexion..."
                NETWORK_WAS_DOWN=1
            fi
            continue
        elif [ $NETWORK_WAS_DOWN -eq 1 ]; then
            log "RED RESTAURADA - Reiniciando camaras..."
            NETWORK_WAS_DOWN=0
            sleep 5
            full_restart
            continue
        ...
        for cam_num in 1 2 3 4 5 6; do
            if check_camera "$cam_num" && check_camera_frozen "$cam_num"; then
                log "CONGELADA (CPU): ${CAM_TITLES[$cam_num]} (CPU baja por ${FROZEN_CYCLES} ciclos)"
                start_camera "$cam_num"
                sleep 2
                fix_window_position "$cam_num"
            fi
        ...
        # ============= CHECK 4: Trafico de red (cada 60s) =============
        if [ $((TRAFFIC_CHECK_COUNTER * WATCHDOG_INTERVAL)) -ge $TRAFFIC_CHECK_INTERVAL ]; then
            TRAFFIC_CHECK_COUNTER=0
            for cam_num in 5 6; do  # Solo camaras VPN
                if check_camera "$cam_num" && check_camera_traffic "$cam_num"; then
                    log "CONGELADA (TRAFICO): ${CAM_TITLES[$cam_num]} sin datos de red"
                    start_camera "$cam_num"
                    sleep 2
                    fix_window_position "$cam_num"
                fi
        ...
        # ============= CHECK 5: Reinicio preventivo VPN (cada 2h) =============
        local now=$(date +%s)
        local since_vpn=$((now - LAST_VPN_RESTART))
        if [ $since_vpn -ge $VPN_RESTART_INTERVAL ]; then
            restart_vpn_cameras
        fi

        # ============= CHECK 6: Reinicio periodico general (cada 4h) =============
...
log "========================================"
log "MATRIX CAMARAS v2.6 - VPN TRAFFIC MONITOR"
log "========================================"
